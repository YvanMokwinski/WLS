...
void wls_residual_manpage(const char * filename_,
		      int * err_)
{
  err_[0] 	= 0;
  ...
  if (!f)
    {
      err_[0] = 1;
      fprintf(stderr,"wls_residual_manpage failed, can not open file '%s'\n",filename_);
      return;
    }
  fprintf(f,".TH wls-residual \"1\" \"" __DATE__ "\"  \"wls-residual description\"\n\n");
  fprintf(f,".SH NAME\n");
  fprintf(f,"\n");
  fprintf(f,"wls-residual\n");
  fprintf(f,"\n");
  fprintf(f,".SH SYNOPSIS\n");
  fprintf(f,"\n");
  fprintf(f,".B wls-residual <matrix_filename> <matrix_filename> <rhs_filename> <sol_filename> [<options>]\n");
  fprintf(f,"\n");
  fprintf(f,".SH DESCRIPTION\n");
  fprintf(f,"\n");
  fprintf(f,".B wls-residual \n");
  fprintf(f,"is a program to calculate the residual from files.\n");
  fprintf(f,"\n");
  ...
  fprintf(f,"\n");
  fprintf(f,".TP\n");
  fprintf(f,".B -o <res_filename>\n");
  fprintf(f,"The name of the file containing the residual\n");
  fprintf(f,"\n");
  fprintf(f,".TP\n");
	 ...
	 char ** 	argv)
{
...

  {
    ...
    if (cmd.get_string("--manpage",filename))
      {
	int err;
	// "doc/man3/wls-residual.3.gz"
	wls_residual_manpage(filename, &err);
	return err;
      }
  }
  ...
  const char * x_filename = cmd.get_arg(3);
  using real_t = double;
  ...
  WLS::dense::matrix z(rhs.m,rhs.n);
  linear_operator->Apply("N",z(0,0),x(0,0));  
  WLS::dense::matrix y(rhs.m,rhs.n);  
  WLS::dense::matrix_h& y_h = static_cast<WLS::dense::matrix_h&>(y);
  WLS::dense::matrix_h& z_h = static_cast<WLS::dense::matrix_h&>(z);
  y_h = rhs;
  y_h -= z_h;
